...
namespace Monorail.Platform
{
    ...
    {
        private byte[] KeyStateThisFrame = new byte[512];
        private byte[] KeyStateLastFrame = new byte[512];

        internal void SetKeyUp(int scanCode)
        {
            if (scanCode >= 0 && scanCode < 512)
            {
                KeyStateThisFrame[scanCode] = 0;
            }
        }

        internal void SetKeyDown(int scanCode)
        {
            if (scanCode >= 0 && scanCode < 512)
            {
                KeyStateThisFrame[scanCode] = 1;
            }
        }
        ...
        internal void Swap()
        {
            for (int i = 0; i < 512; i++)
            {
                KeyStateLastFrame[i] = KeyStateThisFrame[i];
            }
        }

        public bool IsDown(KeyCode scanCode)
        {
            return KeyStateThisFrame[(int)scanCode] == 1;
        }

        public bool IsUp(KeyCode scanCode)
        {
            return KeyStateThisFrame[(int)scanCode] == 0;
        }

        public bool WasPressed(KeyCode scanCode)
        {
            return KeyStateLastFrame[(int)scanCode] == 0 && KeyStateThisFrame[(int)scanCode] == 1;
        }

        public bool WasReleased(KeyCode scanCode)
        {
            return KeyStateLastFrame[(int)scanCode] == 1 && KeyStateThisFrame[(int)scanCode] == 0;
        }
    }
